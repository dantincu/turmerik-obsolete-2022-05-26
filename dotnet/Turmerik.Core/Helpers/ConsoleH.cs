...
namespace Turmerik.Core.Helpers
{
    ...
    {
        public static readonly Lazy<ConsoleH> Instance = new Lazy<ConsoleH>(() => new ConsoleH());
        ...
        public void Write(
            string message, bool writeLine = false, ConsoleColor? consoleColor = null,
            bool colorIsForeGround = true, bool? useBrightColor = null,
            bool resetConsoleAfter = true, bool printTimeStamp = false, params object[] args)
        {
        ...
        public void WriteText(string text, string startText = null,
            string endText = null, ConsoleColorsAgg? consoleColors = null, bool printTimeStamp = false)
        {
            WriteTextCore(text, TextDelims.PlainText,
                startText, endText, consoleColors, printTimeStamp);
        }

        public void WriteMdText(string mdText, string startText = null,
            string endText = null, ConsoleColorsAgg? consoleColors = null, bool printTimeStamp = false)
        {
            WriteTextCore(mdText, TextDelims.MdText,
                startText, endText, consoleColors, printTimeStamp);
        }

        public void WriteJson(object obj, string startText = null,
            string endText = null, ConsoleColorsAgg? consoleColors = null, bool printTimeStamp = false)
        {
            WriteObjCore(
                colors =>
                {
                    ...
                    Write(xml, true, colors.MainForeColor, true, null, false, printTimeStamp);
                },
                consoleColors ?? DefaultColors.Data, startText,
                endText, TextDelims.Json, TextDelims.Data);
        }

        public void WriteException(Exception exc, string startText = null,
            string endText = null, ConsoleColorsAgg? consoleColors = null, bool printTimeStamp = false)
        {
            WriteObjCore(
                colors =>
                {
                    WriteExceptionCore(exc, colors, 0, printTimeStamp);
                },
                consoleColors ?? DefaultColors.Error, startText,
                endText, TextDelims.Json, TextDelims.Data);
        }

        public void WriteLines(int count = 1, bool resetColorsAfter = false)
        {
        ...
        public void WriteWithDelims(string startDelim, string endDelim, string text,
            ConsoleColor textColor, ConsoleColor delimitersBackColor,
            int newLinesBefore = 0, int newLinesAfter = 0, bool printTimeStamp = false)
        {
            WriteLines(newLinesBefore);
            WriteWithDelimsCore(startDelim, delimitersBackColor);

            text = PaddTextIfReq(text);
            Write(text, false, textColor, true, null, false, printTimeStamp);
        ...
        private void WriteTextCore(
            string text, ConsoleTextDelims outterDelims, string startText = null,
            string endText = null, ConsoleColorsAgg? consoleColors = null, bool printTimeStamp = false)
        {
            WriteObjCore(colors =>
                {
                    Write(text, true, colors.MainForeColor, true, null, false, printTimeStamp);
                },
                consoleColors ?? DefaultColors.Data, startText,
                endText, TextDelims.Text, outterDelims);
        }

        private void WriteExceptionCore(Exception exc,
            ConsoleColorsAgg colors, int level = 0, bool printTimeStamp = false)
        {
            ...
            if (level > 0)
            {
                WriteWithDelims("LEVEL", null, level.ToString(), mainColor, highlightColor, 0, 1);
            }

            WriteWithDelims("MESSAGE", null, exc.Message, mainColor, highlightColor, 0, 1);
            WriteWithDelims("TYPE", null, exc.Message, mainColor, highlightColor, 0, 1);
            WriteWithDelims("STACKTRACE", null, exc.StackTrace, mainColor, highlightColor, 0, 1);

            if (exc.InnerException != null)
            {
                WriteWithDelims("INNER EXCEPTION", null, level.ToString(), mainColor, highlightColor, 0, 1);
                WriteExceptionCore(exc.InnerException, colors, level + 1);
            }

            if (exc is AggregateException aggExc)
            {
                WriteWithDelims("INNER EXCEPTIONS (AGGREGATE)", null, level.ToString(), mainColor, highlightColor, 0, 1);

                foreach (var innerExc in aggExc.InnerExceptions)
                {
                    WriteWithDelims("INNER EXCEPTION", null, level.ToString(), mainColor, highlightColor, 0, 1);
                    WriteExceptionCore(innerExc, colors, level + 1);
                }
            }
        }

        private void WriteObjCore(Action<ConsoleColorsAgg> mainAction,
            ConsoleColorsAgg colors, string startText, string endText,
            ConsoleTextDelims mainDelims, ConsoleTextDelims outerDelims, bool printTimeStamp = false)
        {
            ...
            WriteWithDelims(outerDelims.StartDelim, null, startText, textColor, delimsBackColor, 2);
            WriteWithDelims(mainDelims.StartDelim, null, null, textColor, delimsBackColor, 1, 1);

            mainAction(colors);

            WriteWithDelims(mainDelims.EndDelim, null, null, textColor, delimsBackColor, 1, 1);
            WriteWithDelims(null, outerDelims.EndDelim, endText, textColor, delimsBackColor, 0, 2);
        }
            ...
            ConsoleColor delimitersBackColor)
        {
            if (delim != null)
            {
                Write($" <{delim}> ", false, delimitersBackColor, false, null);
            }
        }

        private void WriteCore(Action action, ConsoleColor consoleColor,
            bool colorIsForeGround, bool? useBrightColor, bool resetColorsAfter,
            bool printTimeStamp)
        {
        ...
        private ConsoleColor GetConsoleColor(ConsoleColor consoleColor, bool? useBrightColor)
        {
